...
Open `/atelier usage` for a larger graph inside a padded, rounded border. Use Left/Right to emphasize one child, A to restore all colors, Up/Down or Page Up/Down to page through long legends, and Escape to close. Selecting a child automatically reveals its legend page; paging legends never removes curves. It is a snapshot: close and reopen to refresh. The graph fits the available space without scrolling; very small terminals show a resize hint.
